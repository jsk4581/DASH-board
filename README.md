...
- **Big 3, daily / monthly / yearly**: three cards pinned above every board, each holding up to three items you pick from any board (or the Dump), in a row on a wide screen and flicked a page at a time on a phone, with a dot per card underneath; the items stay where they live and are edited in place
...
- **Diary**: a pinned tab of ruled paper. One sheet per day (today's feedback, gratitude, right after waking), today at the right end with the earlier days lined up to its left on a wide screen and swiped one at a time on a phone; the three standing pages (my future, inner motivation, identity) sit above in one row, and the board's calendar follows underneath. Part of the board document, so it saves, syncs and exports with it
...
- **Big 3**: a pinned card's "Add item" opens a list of every item; tick up to three for that card (an item may sit on more than one card), and the trash on a pinned row unpins it
