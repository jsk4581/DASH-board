...
await render({ size: 1024, scale: 0.9, bg: PAPER, radius: 0.22 }).toFile(out('icon-only.png'))
// Adaptive layers: foreground kept inside the 66% safe zone, plain white background.
await render({ size: 1024, scale: 0.61 }).toFile(out('icon-foreground.png'))
...
await render({ size: 2732, scale: 0.21, bg: PAPER }).toFile(out('splash.png'))
await render({ size: 2732, scale: 0.21, bg: NIGHT, colours: { body: '#ffffff', wing: ACCENT_LIGHT } }).toFile(out('splash-dark.png'))
await writeAndroidLauncher(RES)
